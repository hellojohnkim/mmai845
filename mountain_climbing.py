...
class mountainClimbing(gym.Env):
    def __init__(self,
        ...
        self.transition_table[BASE_CAMP] = {
                CLIMB: {GLACIER: 0.2, BASE_CAMP: 0.8},
                PREPARE: {BASE_CAMP_PREPARED: 1.0}, 
                }
        ...
        self.transition_table[ICE_WALL] = {
                CLIMB: {PEAK: 0.1, ICE_WALL: 0.9},
                PREPARE: {ICE_WALL_PREPARED: 1.0}, 
                }

        self.transition_table[ICE_WALL_PREPARED] = {
                CLIMB: {PEAK: 1.0},
                }
        self.transition_table[PEAK] = {
                CLIMB: {PEAK: 1.0},
                PREPARE: {PEAK: 1.0},
                }
        ...
        self.reward_table = {
            (BASE_CAMP, CLIMB, BASE_CAMP): -1,  # Unsuccessful climb, returns to base
            (BASE_CAMP, CLIMB, GLACIER): 2,  # Successful climb to next stage
            (BASE_CAMP, PREPARE, BASE_CAMP_PREPARED): -0.5,  # Preparation cost
            (BASE_CAMP_PREPARED, CLIMB, GLACIER): 3,  # Prepared climb, higher reward
            (GLACIER, CLIMB, GLACIER): -2,  # Unsuccessful, stays at glacier
            (GLACIER, CLIMB, ICE_WALL): 4,  # Reaches the next difficult stage
            (GLACIER, PREPARE, GLACIER_PREPARED): -0.5,  # Preparation cost
            (GLACIER_PREPARED, CLIMB, ICE_WALL): 5,  # Prepared climb, very positive
            (ICE_WALL, CLIMB, ICE_WALL): -3,  # High risk climb, penalty if failed
            (ICE_WALL, CLIMB, PEAK): 10,  # Reaches the peak, highest reward
            (ICE_WALL, PREPARE, ICE_WALL_PREPARED): -0.5,  # Prepares for final climb
            (ICE_WALL_PREPARED, CLIMB, PEAK): 15  # Prepared final climb to the peak
        }
    ...
    def step(self, action):
        ...
        reward = self.reward_table[(self.state, action, next_state)] 
